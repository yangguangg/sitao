...
class Ticket extends Backend
{
    ...
    /**
     * Ticket模型对象
     * @var \app\admin\model\order\Ticket
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\order\Ticket;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
                    ->with(['indexusers','usercoupons'])
                    ->where($where)
                    ->order($sort, $order)
                    ->paginate($limit);

            foreach ($list as $row) {
                $row->visible(['id','status','order_sn','amount','getting_nums','ticket_title']);
